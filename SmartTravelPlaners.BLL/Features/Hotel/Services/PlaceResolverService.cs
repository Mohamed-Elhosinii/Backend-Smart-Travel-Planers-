...
namespace SmartTravelPlaners.BLL.Features.Hotel.Services
{
    ...
    public class PlaceResolverService : IPlaceResolverService
    {
        ...
        public async Task<PlaceResolutionResult> ResolveAsync(string userInput)
        {
            ...
            // StayAPI did not find the destination (or the call failed).
            // Fall back gracefully: accept the user's input as-is so trip creation
            // is not blocked by an external-API outage or an unlisted city name.
            _logger.LogWarning("StayAPI could not resolve '{Query}'. Falling back to user input as destination.", userInput);
            return PlaceResolutionResult.Exact(normalized, "city", userInput, "fallback");
        }
